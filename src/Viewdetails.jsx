...
const Viewdetails = () => {
  ...
  <div className="lg:max-w-6xl max-w-xl mx-auto">
	...
	{	obj.images.map((item,i)=>{
			return <img key={item} src={item} alt="Product1" className="aspect-[64/85] object-cover object-top w-full cursor-pointer  border-b-2 border-black" />
      ...
      <div className="w-full">
        <div>
          <h3 className="text-lg sm:text-xl font-bold text-gray-800">{obj.tile}</h3>
          <p className="text-gray-500 mt-1 text-sm">{obj.description}
          </p>
          <div className="flex items-center flex-wrap gap-4 mt-4">
            <h4 className="text-gray-800 text-2xl sm:text-3xl font-bold">{(obj.price-(obj.price*obj.discountPercentage/100 )).toFixed(2)}</h4>
            <p className="text-gray-500 text-lg"><strike>${obj.price}</strike> <span className="text-sm ml-1.5">Tax included</span></p>
          </div>
        ...
        <div className="mt-6">
          <div className="flex flex-col items-start">
           ...
           {
			obj.reviews.map((rev)=>{
				return  <div key={rev.reviewerEmail} className="ml-3">
          ...
          <a href="#" className="block text-blue-600 hover:underline text-sm mt-6 font-semibold">Read all
            reviews</a>
        </div>
      </div>
    </div>
